    ...
    <footer className="mt-auto border-t border-[#0F4C45]/10 bg-[#F8F6F1] text-[#0F4C45]">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-12 sm:px-10 sm:py-14 lg:grid-cols-[1.6fr_repeat(4,1fr)] lg:gap-8 lg:px-12">
        <div className="max-w-sm">
          ...
          <p className="mt-5 text-sm leading-6 text-[#0F4C45]/70">
            A considered workspace for focused work, meaningful connections, and steady growth.
          </p>
        </div>
        ...
        <FooterColumn title="Contact">
          <Link href="/contact" className={linkClassName}>Contact Us</Link>
          <a href="tel:+919996169001" className={linkClassName}>99961 69001</a>
          <a href="mailto:mannatcoworking@gmail.com" className={linkClassName}>mannatcoworking@gmail.com</a>
          <a href="https://www.google.com/maps/search/?api=1&query=S1%2C%20Second%20Floor%2C%20Atlas%20Rd%2C%20Parshuram%20Chowk%2C%20Sonipat%2C%20Haryana%20131001" target="_blank" rel="noopener noreferrer" className={linkClassName}>S1, Second Floor, Atlas Rd, Parshuram Chowk, Sonipat, Haryana 131001</a>
        </FooterColumn>
        ...
        <FooterColumn title="Follow">
          <div className="mt-3 flex flex-wrap gap-2">
            {socialLinks.map((social) => (
              <a key={social.label} href={social.href} target="_blank" rel="noopener noreferrer" aria-label={`Open Mannat Coworking on ${social.label}`} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-[#0F4C45]/20 transition-colors duration-200 hover:border-[#C8A96A] hover:bg-[#0F4C45]/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C8A96A] focus-visible:ring-offset-2 focus-visible:ring-offset-[#F8F6F1]">
      ...
      <div className="border-t border-[#0F4C45]/10">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-6 py-5 text-xs text-[#0F4C45]/60 sm:flex-row sm:items-center sm:justify-between sm:px-10 lg:px-12">
          <p>&copy; 2026 Mannat Coworking. All rights reserved.</p>
          <div className="flex flex-wrap gap-x-5 gap-y-1">
            {legalLinks.map((link) => (
              <Link key={link.label} href={link.href} className="transition-colors hover:text-[#0F4C45] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0F4C45]">
                {link.label}
              </Link>
            ))}
          </div>
...
function FooterColumn({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h2 className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#C8A96A]">{title}</h2>
